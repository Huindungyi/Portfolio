Database Engineer | PostgreSQL | MES · APS Operations

Manufacturing systems–focused Database Engineer with hands-on experience
operating MES and APS environments across multiple production sites.

• PostgreSQL-based DB operations ensuring stability, integrity, and performance
• Root-cause analysis of production incidents from a data-first perspective
• Hands-on experience with large-scale manufacturing data and BI workloads
• Strong understanding of MES application logic (C#) and DB integration
• Proven incident response in high-load, mission-critical environments

I focus on preventing operational failures by controlling data state, structure,
and lifecycle 

-----

Database Engineer Portfolio

Operational Stability · Data Integrity · Manufacturing Systems


-----

I. About Me

제조업 환경에서 MES·APS 시스템을 운영하며
PostgreSQL 기반 데이터베이스의 안정성, 정합성, 성능을 중심으로 문제를 해결해 온 DB 엔지니어입니다.

현장 공정과 데이터 흐름의 불일치로 인해 발생하는 다양한 장애를 경험하며,
단순 증상 대응이 아닌 데이터 상태와 구조를 기준으로 한 근본 원인 분석을 수행해 왔습니다.

애플리케이션(C#)과 DB를 함께 이해하는 시야를 바탕으로
운영 환경에서 신뢰할 수 있는 데이터를 유지하는 실전형 DBA로 성장하고자 합니다.

-----

II. Core Strengths

PostgreSQL 기반 MES·APS 운영 경험

대용량 데이터 환경에서의 장애 대응 및 원인 분석

SQL 중심의 데이터 정합성 및 성능 문제 해결

MES 유지보수(C#)를 통한 DB 연계 로직 이해

현장·IT·협력사 간 커뮤니케이션 기반 문제 조율

-----

III. Technical Stack

<DBMS>

PostgreSQL
MES / APS / BI 전반 단일 DBMS 운영
대용량 데이터 환경에서 성능 저하 및 장애 대응
불필요 데이터 정리 및 데이터 정합성 점검
복잡한 제조·경영 데이터 조회 쿼리 작성
장애 원인 분석을 위한 조건 기반 데이터 추적
BI 조회를 고려한 구조 검토 및 MV 설계·운영

<Application (MES Maintenance)>

C#
운영 중인 MES 애플리케이션 유지보수 및 기능 개선
DB 연계 로직 수정 및 SQL 호출 구조 개선
장애 발생 시 코드 레벨 원인 분석 및 대응
DB 구조 변경 시 애플리케이션 영향도 검토 및 반영

<Automation & Operations>

Batch / Python
MV 갱신 및 데이터 정리 작업 자동화
운영 로그 확보 및 주기적 데이터 관리
반복 작업의 안정적 수행을 위한 스크립트 활용

<BI & Data Flow Understanding>

MotionBoard 중심 BI 환경
MES · APS · BI 간 데이터 흐름 전반 이해

-----

IV. Operating Environment Overview

1. Manufacturing Sites

  총 4개 제조 거점의 MES 및 DB 운영 담당
  
  <K 거점>
  
    데이터 규모: 소
    이용자 수: 적음
    시스템 부하: 낮음
    특이사항: 테스트 및 안정성 검증에 유리한 경량 거점
  
  <L 거점>
  
    데이터 규모: 중
    이용자 수: 다수
    시스템 부하: 중간 수준
    특이사항: 일상 운영 및 BI 활용 빈도 높음
  
  <B 거점>
  
    데이터 규모: 대
    이용자 수: 최다
    시스템 부하: 매우 높음
    특이사항: 성능 이슈 및 장애 대응 빈도가 가장 높은 핵심 거점
  
  <M2 거점>
  
    데이터 규모: 최대
    이용자 수: 많음
    시스템 부하: 매우 높음
    특이사항: 타 거점과 상이한 MES 구조를 사용하는 특수 거점

2. System Structure

  K / L / B : 동일한 MES · APS 구조
  M2 : 별도 구조의 MES · APS 운영


3. BI Environment
  
    AppSheet
    Python 기반 자체 BI
    Looker Studio

  Main BI: MotionBoard
    
    실시간 연계성 및 운영 안정성 중시

-----

V. Project Overview

Project 1. 신규 MES 도입 (M1)

  <목적>
  
  MES 미도입 제조 거점에 신규 MES 구축
  업무 표준화 및 시스템 기반 운영 체계 확립
  
  <주요 수행 내용>
  
  현장 워크플로우 분석 및 문서화
  기존 시스템과 신규 MES 병행 운영 가능성 검토
  관리 필요 데이터 항목 도출
  MES·APS 공통 활용 데이터 구조 초안 정의
  
  <역할>
  
  현장 인터뷰 기반 프로세스 분석
  핵심 관리 데이터 정의 참여
  데이터 흐름 관점의 시스템 구조 검토

-----

Project 2. BI 조회 구조 개선 및 MV 구축

  <배경>
  
  실시간 쿼리 기반 BI 구조로 인한 DB 부하 증가
  데이터 볼륨 및 조회 빈도 증가에 따른 성능 저하
  
  <개선 방향>
  
  조회 빈도 높은 BI 대상 MV 도입
  실시간성보다 안정성과 성능 중시
  
  <주요 수행 내용>
  
  기존 BI 쿼리 분석 및 부하 요인 식별
  MV 대상 데이터 선정 및 갱신 주기 설계
  Batch 기반 MV 갱신 및 로그 관리 체계 구성
  
  <효과>
  
  DB 부하 감소
  BI 응답 속도 개선
  운영 안정성 확보

-----

Project 3. MES 시스템 유지보수 및 운영 안정화
  
  <배경>
  
  운영 중인 MES에서 데이터 오류, 비정상 동작, 현장 요구사항 미반영 사례가 지속적으로 발생
  
  <목적>
  
  운영 중인 MES 시스템의 안정성 확보 및 현장-시스템 간 괴리 최소화
  
  <주요 수행 내용>
  
  C# · SQL 기반 MES 기능 수정 및 로직 개선
  현장 요구사항 반영을 위한 기능 추가
  데이터 오류 및 비정상 동작 원인 분석 및 수정
  재발 방지를 고려한 데이터 처리 구조 개선
  
  <역할 및 기여>
  
  장애 발생 시 DB·애플리케이션 연계 관점에서 원인 분석
  DB 구조 변경 시 애플리케이션 영향도 검토 및 반영
  운영 안정성을 기준으로 한 개선 방향 제안
  
  <효과>
  
  반복 발생하던 운영 오류 감소
  현장 업무 흐름과 시스템 간 정합성 개선

-----

Project 4. BI 정보 추출 및 운영 데이터 제공 체계 구축

  <목적>
  
  현장 및 관리자 의사결정을 지원하기 위한 운영·경영 데이터 제공
  
  <배경>
  
  운영 데이터가 DB에는 존재하나, BI 도구에서 바로 활용하기 어려운 구조
  
  <주요 수행 내용>
  
  PostgreSQL 기반 운영·경영 지표 데이터 추출
  MotionBoard 등 BI 도구 활용을 위한 SQL 설계
  현장 및 관리자 요구사항에 맞춘 데이터 제공
  
  <역할 및 기여>
  
  BI 요구사항 정리 및 데이터 구조 검토
  성능과 정합성을 고려한 SQL 작성
  MES·APS·BI 간 데이터 흐름 정리
  
  <효과>
  
  현황 파악을 위한 데이터 접근성 향상
  운영 및 관리 의사결정 지원

-----

VI. Incident Response & Troubleshooting

Case 1. 대용량 데이터 누적으로 인한 MES 접속 불가

  <문제>
  
  MES 전면 무기한 로딩 발생으로 현장 업무 중단
  
  <원인 판단>
  
  최근 데이터 급증 시점 이후 성능 저하가 누적된 점을 근거로
  누적 데이터로 인한 DB 성능 한계 가능성을 우선 가설로 설정
  
  <조치>
  
  서비스 일시 중단 후 공지
  오래된 데이터 백업 및 단계적 삭제 수행
  시스템 정상 작동 여부 우선 확보
  이후 주기적 심야 데이터 정리 정책 수립
  근본 원인 분석은 서비스 안정화 이후 단계적으로 진행
  
  <결과>
  
  MES 정상 접속 복구
  업무 재개 가능 수준으로 성능 안정화
  
  <교훈>
  
  운영 환경에서는 완벽한 원인 규명보다
  신속한 안정화와 데이터 증가를 고려한 사전 관리가 우선됨

-----

Case 2. APS 제조 데이터 미인식 (데이터 포맷 오류)

  <문제>
  
  APS가 제조 데이터를 인식하지 못하며 동작 중단
  
  <원인 판단>
  
  제조 데이터 자체에는 수정 이력이 없어
  기준 정보(날짜·시간) 포맷 오류 가능성을 우선 점검
  
  <조치>
  
  시스템 날짜 기준을 서력 기준으로 통일
  비정상적으로 입력된 날짜 데이터 수정
  APS 데이터 재갱신 수행
  
  <결과>
  
  APS 정상 작동 복구
  
  <교훈>
  
  제조 데이터 외에 기준 정보의 일관성이 시스템 안정성에 직접적인 영향을 미침

-----

Case 3. 공정 취소·중단 데이터로 인한 APS 갱신 오류

  <문제>
  
  APS에서 일부 제조 데이터가 갱신되지 않음
  
  <원인 판단>
  
  공정 취소 및 중단 과정에서
  실제 현장 상태와 DB 상태 간 불일치 가능성을 가설로 설정
  
  <조치>
  
  현장 확인을 통해 제조 완료 여부 및 계획 상태 검증
  불필요한 제조 데이터 정리
  APS 대상 데이터 기준 재정의
  
  <결과>
  
  데이터 정합성 회복
  APS 갱신 로직 정상화
  
  <교훈>
  
  데이터 정합성 관리는
  현장 공정 흐름에 대한 이해를 전제로 이루어져야 함

-----

Case 4. 초기 더미데이터로 인한 APS 오류

  <문제>
  
  APS 제조 계획 수립 중단
  
  <원인 판단>
  
  APS가 참조하는 기간 조건과
  오래된 데이터 간 불일치 가능성을 우선 의심

  <조치>
  
  초기 시스템 구축 시 생성된 더미데이터 확인
  데이터 백업 후 관련 테이블에서 안전하게 삭제
  
  <결과>
  
  APS 즉시 정상화
  제조 계획 기능 복구
  
  <교훈>
  
  데이터는 생성 시점이 아닌
  운영 단계에서의 존재 이유를 기준으로 관리되어야 함

-----

VII. DBA Mindset & Career Direction

  저는 장애나 성능 저하를 마주했을 때
  증상이 아닌 데이터의 상태, 흐름, 기준부터 점검하는 방식으로 문제를 해결해 왔습니다.
  
  MES·APS 운영 환경에서 반복적으로 확인한 사실은
  문제의 대부분이 애플리케이션이 아닌 데이터 구조와 정합성 관리 부재에서 비롯된다는 점이었습니다.
  
  이 경험을 통해 저는 DBA의 역할을
  단순한 DB 운영자가 아닌, 시스템과 현장을 연결하며 서비스의 신뢰성을 책임지는 역할로 인식하게 되었습니다.
  
  앞으로는 PostgreSQL 기반 운영 경험을 더욱 확장하여
  문제가 발생하기 전에 예방할 수 있는 구조와 운영 체계를 설계하는
  신뢰 중심 DBA로 성장하고자 합니다.

-----

VIII. Project Management & Documentation

  Git
  
  SQL, 배치 스크립트, 애플리케이션 연계 로직 변경 이력 관리
  장애 대응 시 수정 내역을 커밋 단위로 추적
  운영 영향이 있는 변경 사항에 대한 명확한 기록 유지
  
  Google Drive
  
  현장 워크플로우 문서
  데이터 구조 정의 및 운영 기준 문서
  협력사·현장 담당자 공유 자료 관리
  
  운영 환경에서는 누가, 언제, 왜 변경했는지를 추적할 수 있어야 한다고 생각합니다.
  코드와 문서를 분리 관리하되, 변경 이력은 반드시 남기는 방식으로 운영 안정성을 유지했습니다.
